...
class CpAgentSkiba2015(CpDifferentialAgentBasis):
    """
    The virtual agent model employing the 2 parameter CP model and Skiba's 2014 recovery kinetics.
    Characteristics:
    ...
    * depleted W' results in exhaustion
    ...
    def _recover(self, p: float):
        ...
        if self._w_exp > 0.1:
            ...
            new_exp = self._w_u * pow(math.e, ((-dcp * (self._hz_t - self._u)) / self._w_p))
            self._w_exp = new_exp
        else:
            self._w_exp = 0
